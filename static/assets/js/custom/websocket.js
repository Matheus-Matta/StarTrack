document.addEventListener('DOMContentLoaded', () => {
  ...
  function prependAlert(note) {
    ...
    item.className = 'd-flex px-4 flex-column py-2 border-bottom cursor-pointer bg-hover-gray-100';
    if (note.link) {
    ...
    item.innerHTML = `
      <div class="d-flex align-items-center">
        ...
        <div>
          ...
          <div class="fs-7 text-gray-500">${note.message}</div>
        </div>
      </div>
    `;
    ...
    if (!item) {
      ...
      item.innerHTML = `
        ...
        <div data-bar class="px-4 mt-2 min-w-425px">
          <div class="progress w-100 h-7px">
  ...
  const icon = document.querySelector('#kt_header_notifications_toggle');
